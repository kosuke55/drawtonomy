...
| **ゴースト** | アクターが*実際に走った*軌跡上に並ぶスタンプ | 再生中の軌跡。trace を読み込めば `driven`、無ければ録画 |
...
- **動く車両**は、作成時のサイズではなくプランナーの `length` x `width` で描画・衝突判定され、
  計画軌跡の幅もそれに従います。
- **ゴースト**のスタンプも同じサイズです。ゴーストは走っている車両そのもののスタンプ
  だからです。
...
アクターは自身の `driven` の状態に沿って動き、そのゴーストも同じ状態に沿って描き直され、
計画軌跡は最新の計画を示します。他に何も読み込む必要はありません。trace が名指ししていない
アクターは録画のゴーストのままです。
